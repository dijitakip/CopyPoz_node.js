...
input bool     EnableWebMonitor  = true;                   // Web İzleme ve Yönetim Aktif
input string   WebMonitorUrl     = "https://fx.haziroglu.com/api/client.php"; // Web API Adresi
input string   RegistrationToken = "MASTER_SECRET_TOKEN_123"; // İlk kayıt için token
...
ulong          g_lastWebHeartbeat = 0;
string         g_webAuthToken = ""; // Sunucudan gelen token
...
int OnInit()
  {
   Print("--- Client EA V4 Başlatılıyor ---");
   ...
   // Web'e Kayıt Ol (İlk Heartbeat)
   if(EnableWebMonitor)
     {
      SendHeartbeat();
     }

   Print("Client EA V4 Başarıyla Başlatıldı. Master Adresi: ", ZmqAddress);
...
void OnDeinit(const int reason)
  {
   Print("--- Client EA V4 Sonlandırılıyor ---");
   EventKillTimer();
   ...
   if(id == CHARTEVENT_OBJECT_CLICK)
     {
      if(sparam == g_btnBuyName)
        ...
        {
         ToggleCopyStatus(!g_copyEnabled);
...
void OnTimer()
  {
   ...
   // Web Heartbeat (5 saniyede bir)
   if(EnableWebMonitor && (GetTickCount() - g_lastWebHeartbeat > 5000))
     {
      SendHeartbeat();
      g_lastWebHeartbeat = GetTickCount();
     }
  }

//+------------------------------------------------------------------+
//| Web API Heartbeat ve Komut Dinleme                               |
//+------------------------------------------------------------------+
void SendHeartbeat()
  {
   string json;
   
   // İlk kayıtta registration_token gönder
   if(g_webAuthToken == "")
     {
      json = StringFormat(
         "{\"account_number\":%I64d,\"account_name\":\"%s\",\"balance\":%.2f,\"equity\":%.2f,\"positions\":%d,\"registration_token\":\"%s\"}",
     ...
     {
      // Sonraki isteklerde auth_token gönder
      json = StringFormat(
         "{\"account_number\":%I64d,\"account_name\":\"%s\",\"balance\":%.2f,\"equity\":%.2f,\"positions\":%d,\"auth_token\":\"%s\"}",
         AccountInfoInteger(ACCOUNT_LOGIN),
         AccountInfoString(ACCOUNT_NAME),
         AccountInfoDouble(ACCOUNT_BALANCE),
         AccountInfoDouble(ACCOUNT_EQUITY),
   ...
   int res = WebRequest("POST", WebMonitorUrl, headers, 2000, data, result, resultHeaders);
   
   if(res == 200)
     {
      // Yanıtı Parse Et (Komut var mı?)
      string response = CharArrayToString(result);
      
      // Token kaydet (ilk kayıtta)
      string token = ExtractJsonString(response, "auth_token");
      if(token != "") g_webAuthToken = token;
      
      // Komut Kontrolü
      string command = ExtractJsonString(response, "command");
      if(command != "")
        {
         Print("Web'den Komut Alındı: ", command);
         
         if(command == "PAUSE") ToggleCopyStatus(false);
         else if(command == "RESUME") ToggleCopyStatus(true);
         else if(command == "CLOSE_ALL_BUY") CloseAllPositions(POSITION_TYPE_BUY);
         else if(command == "CLOSE_ALL_SELL") CloseAllPositions(POSITION_TYPE_SELL);
     ...
     {
      // if(LogDetailed) Print("Web Heartbeat Hatası: ", res);
     }
  }
